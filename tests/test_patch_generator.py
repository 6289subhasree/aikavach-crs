...
def valid_edit() -> dict[str, object]:
    return {
...
def test_fake_client_returns_valid_patch_proposal() -> None:
    ...
    client = FakePatchLLMClient(valid_edit())
    ...
    assert result.rationale == reasoning.remediation_strategy
...
def test_generator_rejects_multiline_replacement() -> None:
    ...
    with pytest.raises(PatchGenerationError, match="exactly one line"):
        PatchGenerator(FakePatchLLMClient(edit)).generate(
...
def test_generator_rejects_noop_replacement() -> None:
    finding, reasoning, context = inputs()
    edit = {"replacement_line": "subprocess.run(command, shell=True, check=False)"}
...
def test_generator_rejects_extra_model_metadata() -> None:
    finding, reasoning, context = inputs()
    edit = valid_edit()
    edit["target_file"] = "unrelated.py"

    with pytest.raises(PatchGenerationError, match="malformed structured output"):
        PatchGenerator(FakePatchLLMClient(edit)).generate(
            finding, reasoning, context
        )
...
def test_generator_rejects_mismatched_reasoning() -> None:
    ...
    with pytest.raises(PatchGenerationError, match="Reasoning finding_id"):
        PatchGenerator(FakePatchLLMClient(valid_edit())).generate(
            finding, reasoning, context
        )
